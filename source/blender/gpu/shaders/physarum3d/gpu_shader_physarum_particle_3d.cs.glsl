...
[numthreads(10, 10, 10)] void main(uint index
  ...
  float3 off_center_base_dir = float3(
...
#define SAMPLE_POINTS 8
  ...
  float3 off_center_base_dir_turn = float3(
      sin(theta_turn) * cos(ph), cos(theta_turn), sin(theta_turn) * sin(ph));
  if (direction > 0) {
    float3 best_direction = rotate(off_center_base_dir_turn,
                                   center_axis,
  ...
  float3 dp = float3(sin(t) * cos(ph), cos(t), sin(t) * sin(ph)) * move_distance *
              (move_sense_offset + max_value * move_sense_coef);
  x += dp.x;
  y += dp.y;
  z += dp.z;
